        ...
        try:
            ...
            if Config.FORWARD_AS_COPY is True:
                ...
                send_stored_file = await bot.forward_messages(chat_id=cmd.from_user.id, from_chat_id=Config.DB_CHANNEL,
                                                              message_ids=file_id)
            await send_stored_file.reply_text(f"Selamat Menikmati (.づ◡﹏◡)づ."
                )
        except Exception as err:
            await cmd.reply_text(f"Eror coba lagi beberapa saat")
...
async def main(bot: Client, message: Message):
    if message.chat.type == "private":
        ...
        if Config.OTHER_USERS_CAN_SAVE_FILE is False:
            return
        editable = await message.reply_text("Tunggu ya beb! ...")
        try:
